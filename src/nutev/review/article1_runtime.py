"""Article 1 runtime on top of the existing Evidence Matrix SQLite.

This module implements the remaining Article 1 execution objects without
creating a parallel engine:

- P2: persistent ABCD-NutEV 34/34 double extraction + adjudication;
- P3: explicit relation ledger + evidence instances + descriptive calibration;
- P4: family-preserving synthesis, Sheet-sync payload and citation-grade manifest.

Scientific boundaries remain strict. Software capability does not close PRESS,
GF gates, reviewer identity, freeze, formal search, screening, PRISMA or any
scientific result. Human decisions remain authoritative.
"""
...
def _normalize_slot(value: object) -> str:
    ...
    aliases = {"R1": "REVIEWER_1", "R2": "REVIEWER_2"}
...
def initialize_article1_runtime(db_path: Path) -> None:
    """Add Article 1 tables to the same Evidence Matrix database."""
    initialize(db_path)
    with _db(db_path) as con:
        con.executescript(
            ...
            CREATE TABLE IF NOT EXISTS article1_reviewer_assignments(
              ...
              gf07_resolved INTEGER NOT NULL DEFAULT 0 CHECK(gf07_resolved IN (0,1)),
            ...
            CREATE TABLE IF NOT EXISTS article1_abcd_submissions(
              ...
              action_strategy TEXT NOT NULL DEFAULT '',
              target TEXT NOT NULL DEFAULT '',
              actor_responsible TEXT NOT NULL DEFAULT '',
              frequency_sequence TEXT NOT NULL DEFAULT '',
              tool_material TEXT NOT NULL DEFAULT '',
              indicator_criterion TEXT NOT NULL DEFAULT '',
              context_condition TEXT NOT NULL DEFAULT '',
              interpretation_nature TEXT NOT NULL DEFAULT '',
              codebook_version TEXT NOT NULL,
              revision INTEGER NOT NULL,
              submitted_at TEXT NOT NULL,
              FOREIGN KEY(session_id) REFERENCES screening_sessions(session_id),
              UNIQUE(session_id,document_id,article_id,reviewer_slot,code,revision));
            ...
            CREATE TABLE IF NOT EXISTS article1_abcd_adjudications(
              ...
              FOREIGN KEY(session_id) REFERENCES screening_sessions(session_id),
              UNIQUE(session_id,document_id,article_id,code,revision));
            ...
            CREATE TABLE IF NOT EXISTS article1_relation_submissions(
              ...
              submitted_at TEXT NOT NULL,
              FOREIGN KEY(session_id) REFERENCES screening_sessions(session_id),
              UNIQUE(session_id,document_id,article_id,reviewer_slot,relation_key,revision));
            ...
            CREATE TABLE IF NOT EXISTS article1_relation_adjudications(
              ...
              decided_at TEXT NOT NULL,
              FOREIGN KEY(session_id) REFERENCES screening_sessions(session_id),
              UNIQUE(session_id,document_id,article_id,relation_key,revision));
...
def set_article1_reviewer_assignment(
    ...
    *,
    ...
    """Record the GF-07 human identities; no identity is synthesized by software."""
        ...
        con.execute(
            """INSERT INTO article1_reviewer_assignments(
               ...
               ON CONFLICT(session_id) DO UPDATE SET
                 ...
                 gf07_resolved=1,notes=excluded.notes,
...
def article1_reviewer_assignment(db_path: Path, session_id: str) -> dict[str, Any] | None:
    initialize_article1_runtime(db_path)
    with _db(db_path) as con:
        row = con.execute(
            "SELECT * FROM article1_reviewer_assignments WHERE session_id=?",
...
def _formal_guard(
    ...
    *,
    ...
    if not assignment or not bool(assignment["gf07_resolved"]):
        raise ValueError("FORMAL Article 1 work is blocked until GF-07 has real R1/R2/adjudicator identities")
    if reviewer_slot and reviewer_name:
        expected = assignment["reviewer_1_name"] if reviewer_slot == "REVIEWER_1" else assignment["reviewer_2_name"]
        if _clean(reviewer_name).casefold() != _clean(expected).casefold():
            raise ValueError(f"{reviewer_slot} identity does not match the GF-07 assignment")
...
def submit_article1_abcd(
    ...
    *,
    ...
    frequency_sequence: str = "",
    tool_material: str = "",
    indicator_criterion: str = "",
    context_condition: str = "",
    interpretation_nature: str = "",
) -> dict[str, Any]:
    """Append one human reviewer decision for one ABCD component."""
    initialize_article1_runtime(db_path)
    ...
    if normalized_presence == "YES" and not _clean(evidence):
        raise ValueError("YES requires traceable supporting evidence; machine suggestion alone is insufficient")
    with _db(db_path) as con:
        revision = int(
            con.execute(
                """SELECT COALESCE(MAX(revision),0)+1 FROM article1_abcd_submissions
        ...
        con.execute(
            ...
            (
                ...
                locator.strip(),
                evidence.strip(),
                action_strategy.strip(),
                target.strip(),
                actor_responsible.strip(),
                frequency_sequence.strip(),
                tool_material.strip(),
                indicator_criterion.strip(),
                context_condition.strip(),
                interpretation_nature.strip(),
                ABCD_VERSION,
                revision,
                _now(),
            ),
        )
        row = con.execute("SELECT * FROM article1_abcd_submissions WHERE id=?", (row_id,)).fetchone()
    return dict(row)


def instantiate_article1_abcd_grid(document_id: str) -> list[dict[str, Any]]:
    """Return the exact 34 expected rows; blank means unassessed, never absence."""
    return [
        {
    ...
    with _db(db_path) as con:
        rows = con.execute(
            f"""SELECT a.* FROM article1_abcd_submissions a JOIN(
                ...
                AND x.reviewer_slot=a.reviewer_slot AND x.code=a.code AND x.revision=a.revision
...
def compare_article1_abcd(db_path: Path, *, session_id: str, document_id: str) -> list[dict[str, Any]]:
    latest = _latest_abcd(db_path, session_id=session_id, document_id=document_id)
    ...
    for code in ABCD_CODES:
        ...
        final = (
            {key: r1.get(key) for key in ("code", "presence", "depth", *ABCD_DETAIL_FIELDS)}
            if status == "AGREED" and r1
            else adjudication.get("final") if adjudication else None
        )
        output.append(
            {
                "code": code,
                ...
                "status": status,
                "final": final,
                "final_status": "AGREED" if status == "AGREED" else "ADJUDICATED" if adjudication else "PENDING",
            }
        )
    return output
...
def adjudicate_article1_abcd(
    ...
    **details: str,
    ...
    assignment = article1_reviewer_assignment(db_path, session_id)
    if assignment and _clean(name).casefold() != _clean(assignment["adjudicator_name"]).casefold():
    ...
    comparison = {row["code"]: row for row in compare_article1_abcd(db_path, session_id=session_id, document_id=document_id)}[normalized_code]
    if comparison["status"] not in {"DIVERGENT", "UNRESOLVED_DOUBT"}:
        raise ValueError("only divergent or unresolved-DOUBT ABCD components require adjudication")
    ...
    if presence == "YES" and not final["evidence"]:
        r1 = comparison.get("reviewer_1") or {}
        r2 = comparison.get("reviewer_2") or {}
        final["evidence"] = _clean(r1.get("evidence") or r2.get("evidence"))
    if presence == "YES" and not final["evidence"]:
        raise ValueError("final YES adjudication requires traceable evidence")
    with _db(db_path) as con:
        revision = int(
            con.execute(
                """SELECT COALESCE(MAX(revision),0)+1 FROM article1_abcd_adjudications
        ...
        row = con.execute("SELECT * FROM article1_abcd_adjudications WHERE id=?", (row_id,)).fetchone()
    return dict(row)


def final_article1_abcd(db_path: Path, *, session_id: str, document_id: str) -> list[dict[str, Any]]:
    comparison = compare_article1_abcd(db_path, session_id=session_id, document_id=document_id)
    pending = [row["code"] for row in comparison if row["final_status"] == "PENDING"]
    if pending:
        raise ValueError(f"ABCD extraction is not closed; pending components: {', '.join(pending)}")
    rows = [dict(row["final"] or {}) for row in comparison]
    assert_document_can_close(rows)
    return rows


def article1_abcd_document_status(db_path: Path, *, session_id: str, document_id: str) -> dict[str, Any]:
    comparison = compare_article1_abcd(db_path, session_id=session_id, document_id=document_id)
    ...
    return {
        **raw_status,
        ...
        "agreed_components": sum(row["final_status"] == "AGREED" for row in comparison),
        "adjudicated_components": sum(row["final_status"] == "ADJUDICATED" for row in comparison),
...
def article1_abcd_calibration_report(
    ...
    *,
    ...
    for document_id in document_ids:
        for row in compare_article1_abcd(db_path, session_id=session_id, document_id=document_id):
            r1 = row["reviewer_1"] or {}
            r2 = row["reviewer_2"] or {}
            pairs.append(
                {
                    "code": row["code"],
...
def normalize_relation(
    *, source_code: object, target_code: object, direction: object, relation_type: object
    ...
    if source not in ABCD_COMPONENTS or target not in ABCD_COMPONENTS:
        raise ValueError("source_code and target_code must be valid ABCD component codes")
    if source == target:
...
def submit_article1_relation(
    ...
    *,
    ...
    """Record one explicit relation. Co-occurrence alone is never accepted."""
    initialize_article1_runtime(db_path)
    _require_included(db_path, session_id, document_id, ARTICLE1_ID)
    _open_session(db_path, session_id)
    slot = _normalize_slot(reviewer_slot)
    name, role = _reviewer(reviewer_name, reviewer_role)
    mode = _normalize_mode(execution_mode)
    _formal_guard(db_path, session_id=session_id, execution_mode=mode, reviewer_slot=slot, reviewer_name=name)
    relation_key, source, target, normalized_direction, normalized_type = normalize_relation(
    ...
    if not cleaned_evidence:
        raise ValueError("an explicit relation requires at least one traceable evidence passage")
    with _db(db_path) as con:
        revision = int(
            con.execute(
                """SELECT COALESCE(MAX(revision),0)+1 FROM article1_relation_submissions
                   WHERE session_id=? AND document_id=? AND reviewer_slot=? AND relation_key=?""",
        ...
        row = dict(con.execute("SELECT * FROM article1_relation_submissions WHERE id=?", (row_id,)).fetchone())
        evidence_rows = con.execute(
            "SELECT locator,evidence,evidence_order FROM article1_relation_evidence_instances WHERE relation_submission_id=? ORDER BY evidence_order",
            (row_id,),
        ).fetchall()
    ...
    with _db(db_path) as con:
        rows = con.execute(
            f"""SELECT r.* FROM article1_relation_submissions r JOIN(
                ...
                AND x.reviewer_slot=r.reviewer_slot AND x.relation_key=r.relation_key AND x.revision=r.revision
                WHERE {' AND '.join(clauses)}""",
            params,
        ).fetchall()
        output: list[dict[str, Any]] = []
    ...
    with _db(db_path) as con:
        rows = con.execute(
            """SELECT a.* FROM article1_relation_adjudications a JOIN(
                 SELECT relation_key,MAX(revision) revision FROM article1_relation_adjudications
...
def compare_article1_relations(db_path: Path, *, session_id: str, document_id: str) -> list[dict[str, Any]]:
    latest = _latest_relations(db_path, session_id=session_id, document_id=document_id)
    by_slot: dict[str, dict[str, dict[str, Any]]] = {slot: {} for slot in SLOTS}
    for row in latest:
    ...
    output = []
    for key in keys:
        r1, r2 = by_slot["REVIEWER_1"].get(key), by_slot["REVIEWER_2"].get(key)
        if r1 and r2:
            ...
            final = {
                ...
                "evidence_instances": (r1.get("evidence_instances") or []) + (r2.get("evidence_instances") or []),
            }
            final_status = "AGREED"
        else:
            status = "REVIEWER_SET_DIVERGENCE"
            adj = adjudicated.get(key)
            final = adj.get("final") if adj and adj["final_decision"] == "INCLUDE" else None
            final_status = "ADJUDICATED_INCLUDE" if final else "ADJUDICATED_EXCLUDE" if adj else "PENDING"
        output.append(
            {
                "relation_key": key,
                "reviewer_1": r1,
...
def adjudicate_article1_relation(
    ...
    *,
    ...
    comparison = {row["relation_key"]: row for row in compare_article1_relations(db_path, session_id=session_id, document_id=document_id)}.get(relation_key)
    if not comparison or comparison["status"] != "REVIEWER_SET_DIVERGENCE":
        raise ValueError("only reviewer-set relation divergences require adjudication")
    ...
    with _db(db_path) as con:
        revision = int(
            con.execute(
                """SELECT COALESCE(MAX(revision),0)+1 FROM article1_relation_adjudications
        ...
        row = con.execute("SELECT * FROM article1_relation_adjudications WHERE id=?", (row_id,)).fetchone()
    return dict(row)


def final_article1_relations(db_path: Path, *, session_id: str, document_id: str) -> list[dict[str, Any]]:
    comparison = compare_article1_relations(db_path, session_id=session_id, document_id=document_id)
    pending = [row["relation_key"] for row in comparison if row["final_status"] == "PENDING"]
    if pending:
        raise ValueError("relation extraction is not closed; unresolved reviewer-set divergences remain")
...
def article1_relation_calibration_report(
    ...
    *,
    ...
    for document_id in document_ids:
        for row in _latest_relations(db_path, session_id=session_id, document_id=document_id):
            pair = (document_id, row["relation_key"])
            (r1 if row["reviewer_slot"] == "REVIEWER_1" else r2).add(pair)
    intersection = r1 & r2
    union = r1 | r2
    ...
    return {
        ...
        "interpretation": "Descriptive relation-set agreement only. No Jaccard pass threshold; absent 34x34 pairs are not negatives.",
    }
...
def save_article1_method_characterization(
    ...
    *,
    ...
    with _db(db_path) as con:
        revision = int(
            con.execute(
                "SELECT COALESCE(MAX(revision),0)+1 FROM article1_method_characterization WHERE session_id=? AND document_id=?",
        ...
        con.execute(
            ...
            (row_id, session_id, document_id, family.strip(), _j(dict(fields)), name, role, revision, _now()),
        )
        row = con.execute("SELECT * FROM article1_method_characterization WHERE id=?", (row_id,)).fetchone()
    return dict(row)


def _latest_method_characterization(db_path: Path, session_id: str) -> list[dict[str, Any]]:
    initialize_article1_runtime(db_path)
    with _db(db_path) as con:
        rows = con.execute(
            """SELECT m.* FROM article1_method_characterization m JOIN(
                 SELECT session_id,document_id,MAX(revision) revision FROM article1_method_characterization
                 WHERE session_id=? GROUP BY session_id,document_id) x
               ON x.session_id=m.session_id AND x.document_id=m.document_id AND x.revision=m.revision""",
            (session_id,),
        ).fetchall()
    output=[]
    for raw in rows:
        row=dict(raw); row["fields"]=_loads(row.pop("fields_json"),{}); output.append(row)
    return output


def _article1_included_documents(db_path: Path, session_id: str) -> list[dict[str, Any]]:
    return list(_included(db_path, session_id, ARTICLE1_ID))


def article1_runtime_status(db_path: Path, *, session_id: str) -> dict[str, Any]:
    documents = _article1_included_documents(db_path, session_id)
    per_document=[]
    for row in documents:
        document_id=row["document_id"]
        abcd=article1_abcd_document_status(db_path,session_id=session_id,document_id=document_id)
        relation_comparison=compare_article1_relations(db_path,session_id=session_id,document_id=document_id)
        relation_pending=sum(item["final_status"]=="PENDING" for item in relation_comparison)
        per_document.append({
            "document_id":document_id,
            "family":row.get("family", row.get("document_family", "")),
            "abcd_closed":bool(abcd["closed"]),
            "abcd_pending":len(abcd["pending_codes"]),
            "relation_pending":relation_pending,
            "relations_closed":relation_pending==0,
        })
    ready=bool(documents) and all(row["abcd_closed"] and row["relations_closed"] for row in per_document)
    return {
        "session_id":session_id,
...
def article1_synthesis(db_path: Path, *, session_id: str, strict: bool = True) -> dict[str, Any]:
    documents = _article1_included_documents(db_path, session_id)
    status = article1_runtime_status(db_path, session_id=session_id)
    if strict and not status["synthesis_ready"]:
        raise ValueError("Article 1 synthesis is blocked until every included document has closed ABCD and relation review")
    component_family: dict[tuple[str, str], Counter] = defaultdict(Counter)
    depth_family: dict[tuple[str, str], Counter] = defaultdict(Counter)
    ...
    for source in documents:
        ...
        family=_clean(source.get("family") or source.get("document_family") or "UNSPECIFIED") or "UNSPECIFIED"
        try:
            abcd=final_article1_abcd(db_path,session_id=session_id,document_id=document_id)
        except ValueError:
            if strict: raise
            continue
        closed_documents += 1
        ...
        try:
            relations=final_article1_relations(db_path,session_id=session_id,document_id=document_id)
        except ValueError:
            if strict: raise
            relations=[]
    ...
    cooccurrence=[
        {"family":family,"source_code":source,"target_code":target,"documents_with_cooccurrence":count}
        for (family,source,target),count in sorted(cooccurrence_family.items())
    ]
    relations=[]
    for (family,key),count in sorted(relation_family.items()):
        source,target,direction,kind=key.split("|",3)
        relations.append({
            "family":family,"relation_key":key,"source_code":source,"target_code":target,
            "direction":direction,"relation_type":kind,"documents_with_explicit_relation":count,
        })
    return {
        "session_id":session_id,
        "codebook_version":ABCD_VERSION,
        ...
        "method_characterization":_latest_method_characterization(db_path,session_id),
        "guardrails":{
...
def create_article1_synthesis_snapshot(db_path: Path, *, session_id: str, strict: bool = True) -> dict[str, Any]:
    initialize_article1_runtime(db_path)
    ...
    with _db(db_path) as con:
        con.execute(
            ...
            (snapshot_id,session_id,_now(),ABCD_VERSION,serialized,digest,int(bool(payload["ready"]))),
        )
    return {"snapshot_id":snapshot_id,"payload_sha256":digest,"payload":payload}
...
def assert_article1_prisma_eligible(
    *,
    ...
    """Hard guard only; this function does not manufacture PRISMA counts."""
    mode=_normalize_mode(execution_mode)
    if mode != "FORMAL":
        raise ValueError("PILOT/STAGING/CALIBRATION contribute zero formal PRISMA counts")
    ...
    if not screening_calibration_released:
        raise ValueError("formal PRISMA is blocked until screening calibration is released")
    if int(unresolved_title_abstract) or int(unresolved_full_text):
        raise ValueError("final PRISMA included counts are blocked by unresolved screening decisions")


def article1_sheet_payload(db_path: Path, *, session_id: str, strict: bool = False) -> dict[str, Any]:
    """Build a controlled payload for the canonical Google Sheet audit surface.

    The payload is export-only: it does not authorize Sheet -> runtime overwrite.
    """
    status=article1_runtime_status(db_path,session_id=session_id)
    submissions=_latest_abcd(db_path,session_id=session_id)
    relations=_latest_relations(db_path,session_id=session_id)
    comparisons=[]
    ...
    for document in status["documents"]:
        ...
        comparisons.extend({"document_id":document_id,**row} for row in compare_article1_abcd(db_path,session_id=session_id,document_id=document_id))
        relation_comparisons.extend({"document_id":document_id,**row} for row in compare_article1_relations(db_path,session_id=session_id,document_id=document_id))
        if document["abcd_closed"]:
            final_abcd.extend({"document_id":document_id,**row} for row in final_article1_abcd(db_path,session_id=session_id,document_id=document_id))
        if document["relations_closed"]:
            final_relations.extend({"document_id":document_id,**row} for row in final_article1_relations(db_path,session_id=session_id,document_id=document_id))
    synthesis=article1_synthesis(db_path,session_id=session_id,strict=strict)
    return {
        "sync_direction":"ENGINE_TO_SHEET",
        "runtime_is_authoritative":True,
        "tabs":{
            "08_CODEBOOK_ABCD":codebook_rows(),
            "09E_RESULTADOS_CONCORDANCIA":[],
            "10_EXTRACAO_ABCD":final_abcd,
            "10A_RELACOES_ABCD":final_relations,
            "11_DIVERGENCIAS":[row for row in comparisons if row["final_status"]=="PENDING"] + [row for row in relation_comparisons if row["final_status"]=="PENDING"],
        ...
        "audit":{
            ...
            "session_id":session_id,
            "status":status,
            "raw_abcd_submissions":len(submissions),
            "raw_relation_submissions":len(relations),
            "generated_at":_now(),
        },
    }
...
def article1_manifest(
    ...
    *,
    ...
    status=article1_runtime_status(db_path,session_id=session_id)
    assignment=article1_reviewer_assignment(db_path,session_id)
    resolved_sha=_clean(git_sha) or _clean(os.environ.get("GITHUB_SHA"))
    payload={
        ...
        "gf07":{
            ...
            "reviewer_1":assignment.get("reviewer_1_name","") if assignment else "",
            "reviewer_2":assignment.get("reviewer_2_name","") if assignment else "",
            "adjudicator":assignment.get("adjudicator_name","") if assignment else "",
        },
        "runtime_status":status,
        "scientific_claims_not_implied":[
            "PRESS approved","GF-01/02/03/10 closed","formal search executed",
            "formal PRISMA complete","scientific results valid","human decisions automated",
        ],
    }
...
def article1_export_bundle(db_path: Path, *, session_id: str) -> dict[str, Any]:
    """Return datasets for Evidence Matrix export without inventing final results."""
    status=article1_runtime_status(db_path,session_id=session_id)
    ...
    for document in documents:
        document_id=document["document_id"]
        abcd_comparison.extend({"document_id":document_id,**row} for row in compare_article1_abcd(db_path,session_id=session_id,document_id=document_id))
        relation_comparison.extend({"document_id":document_id,**row} for row in compare_article1_relations(db_path,session_id=session_id,document_id=document_id))
        if document["abcd_closed"]:
            final_abcd.extend({"document_id":document_id,**row} for row in final_article1_abcd(db_path,session_id=session_id,document_id=document_id))
        if document["relations_closed"]:
            final_relations.extend({"document_id":document_id,**row} for row in final_article1_relations(db_path,session_id=session_id,document_id=document_id))
    return {
        ...
        "sheet_payload":article1_sheet_payload(db_path,session_id=session_id,strict=False),
