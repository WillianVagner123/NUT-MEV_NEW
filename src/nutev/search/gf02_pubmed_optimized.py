...
def _audit_reusable(payload: dict[str, Any], query: str) -> bool:
    return bool(
        payload
        and payload.get("status") in {"completed", "empty"}
        and str(payload.get("exact_query") or "") == query
        and str(payload.get("query_sha256") or "") == _hash_text(query)
    )
...
def run_gf02_pubmed_pilot(
    ...
    *,
    ...
    """Execute the current GF-02 PubMed PILOT with durable unit checkpoints.

    Required lines use PubMed ESearch ``retmax=0``. Only the rescue-only slice
    required for human review is downloaded. When ``resume`` is true and the
    caller reuses the same ``run_id``, completed line counts, the rescue sample,
    and individual sentinel probes are reused from disk after query-hash checks.
    Scientific semantics and PILOT/PRISMA guards are unchanged.
    """
    repo = Path(repo_root)
    ...
    if (
        existing_manifest.get("status") == "SUCCEEDED"
        and existing_manifest.get("candidate_version") == candidate_version
        and existing_manifest.get("candidate_config_sha256") == _hash_file(config_path)
    ):
        _emit(progress_fn, f"GF-02 {candidate_version}: execução já concluída; reutilizando checkpoint final.")
    ...
    _emit(progress_fn, f"GF-02 {candidate_version}: iniciando/retomando PILOT PubMed.")
    for index, line_id in enumerate(count_lines, start=1):
        audit_path = run_dir / f"{line_id.replace('#', 'line_')}.audit.json"
        cached = _read_json(audit_path) if resume else {}
        if _audit_reusable(cached, expressions[line_id]):
            audit = cached
            reused_units += 1
            _emit(
                progress_fn,
                f"{line_id} ({index}/{len(count_lines)}): checkpoint reutilizado ({audit.get('total_found')} registros).",
            )
        else:
            ...
            _emit(progress_fn, f"{line_id}: {audit['total_found']} registros; checkpoint salvo.")
    ...
    rescue_audit_path = run_dir / f"{version_slug}_rescue_only.audit.json"
    sample_path = run_dir / f"rescue_only_sample_{version_slug}.csv"
    cached_rescue = _read_json(rescue_audit_path) if resume else {}
    if _audit_reusable(cached_rescue, rescue_query) and sample_path.is_file():
        rescue = cached_rescue
        actual_sample = _sample_row_count(sample_path)
        if actual_sample > 0:
            reused_units += 1
            _emit(progress_fn, f"Amostra rescue-only: checkpoint reutilizado ({actual_sample} registros).")
        else:
            cached_rescue = {}

    if not (_audit_reusable(cached_rescue, rescue_query) and sample_path.is_file() and _sample_row_count(sample_path) > 0):
        _emit(
            progress_fn,
            f"Baixando somente {sample_size} registros do rescue-only para revisão humana...",
        )
        ...
        _write_json(rescue_audit_path, rescue)
        _, actual_sample = _write_sample(
            sample_path,
            rescue_rows,
            sample_size,
            noise_seed,
        ...
        _emit(progress_fn, f"Amostra rescue-only pronta: {actual_sample} registros; checkpoint salvo.")
    else:
        rescue = cached_rescue
        actual_sample = _sample_row_count(sample_path)
    ...
    for sentinel_id in priority_ids:
        ...
        for line_id in _MECHANISM_LINES:
            ...
            probe_checkpoint = checkpoint_dir / f"probe_{sentinel_id}_{line_id.replace('#', 'line_')}.json"
            cached_probe = _read_json(probe_checkpoint) if resume else {}
            reusable_probe = bool(
                query
                and cached_probe
                and cached_probe.get("status") in {"completed", "empty"}
                and str(cached_probe.get("query_sha256") or "") == _hash_text(query)
            )
            if reusable_probe:
                probe = cached_probe
                reused_units += 1
                _emit(
                    progress_fn,
                    f"Sentinela {sentinel_id} em {line_id} ({probe_index}/{total_probes}): checkpoint reutilizado.",
                )
            else:
                _emit(
                    progress_fn,
                    f"Sentinela {sentinel_id} em {line_id} ({probe_index}/{total_probes})...",
                )
                if not query:
                    probe = {
                ...
                _write_json(probe_checkpoint, probe)
    ...
    manifest = {
        ...
        "execution_plan": "COUNT_FIRST_SAMPLE_ONLY_RESUMABLE",
        "resume_enabled": bool(resume),
        "checkpoint_units_reused": reused_units,
        ...
        "rescue_only_sampling_rule": {
            ...
            "note": (
                "PILOT retrieves only the deterministic human-review slice; "
                "it does not claim representativeness of the full rescue-only set."
            ),
        },
    ...
    _emit(progress_fn, f"GF-02 {candidate_version}: {manifest['status']}; checkpoint final salvo.")
    return manifest
