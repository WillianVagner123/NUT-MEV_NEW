...
from nutev.search.pubmed import PubMedClient, _request_json
...
def _version_slug(version: str) -> str:
    slug = re.sub(r"[^a-zA-Z0-9]+", "_", version.strip()).strip("_").lower()
    if not slug:
        raise ValueError("GF-02 PubMed current_candidate cannot be blank")
    return slug
...
def load_candidate_config(path: Path) -> dict[str, Any]:
    ...
    current_candidate = str(data.get("current_candidate") or "").strip()
    if not current_candidate or data.get("final_line") != "#7":
        raise ValueError("GF-02 PubMed requires a current candidate with final line #7")
    expected_counts = ["#1", "#2", "#3", "#4", "#6", "#7"]
    if data.get("required_count_lines") != expected_counts:
        raise ValueError("GF-02 PubMed requires counts for #1,#2,#3,#4,#6,#7")
    expressions = resolved_line_expressions(data)
    if any(line_id not in expressions for line_id in expected_counts):
        raise ValueError("GF-02 PubMed required line missing")
    unexpected = sorted(set(re.findall(r"[a-z]+", expressions["#4"].casefold())) - _ALLOWED_RESCUE_WORDS)
    if unexpected:
        raise ValueError("GF-02 PubMed rescue is not condition-neutral: " + ", ".join(unexpected))
    ...
    for version, payload in (data.get("superseded_candidates") or {}).items():
        if version == current_candidate:
            raise ValueError("current GF-02 candidate cannot also be superseded")
...
def _search(query: str, limit: int, context: dict[str, Any]) -> ProviderResult:
    ...
    if not meta.get("querytranslation") and not meta.get("query_translation"):
        try:
            payload = _request_json("esearch.fcgi", {"db": "pubmed", "retmode": "json", "term": query, "retmax": 0})
            ...
            meta["querytranslation_capture_error"] = str(exc)
        result.meta = meta
    return result


def _boolean_semantics_warning(meta: dict[str, Any]) -> str:
    warninglist = meta.get("warninglist") or {}
    if not warninglist:
        return ""
    ignored = warninglist.get("phrasesignored") or warninglist.get("phrases_ignored") or []
    if isinstance(ignored, str):
        ignored = [ignored]
    boolean_tokens = {"AND", "OR", "NOT"}
    affected = [str(item) for item in ignored if str(item).strip().upper() in boolean_tokens]
    if affected:
        return "PubMed ignored Boolean operator(s): " + ", ".join(affected)
    return ""
...
def _write_sample(path: Path, rows: list[dict[str, Any]], size: int, seed: int, total: int | None, returned: int, capped: bool, *, candidate_version: str) -> tuple[Path, int]:
    ...
    with path.open("w", encoding="utf-8-sig", newline="") as handle:
        ...
        for idx, row in enumerate(ordered, 1):
            writer.writerow({
                ...
                "provider": "PUBMED", "strategy_version": f"B-NORM-PUBMED-{candidate_version}-rescue-only", "sampling_rule": rule,
...
def run_gf02_pubmed_pilot(repo_root: Path, *, project_root: Path, limit: int = 10000, noise_sample_size: int | None = None, noise_seed: int = 20260812, search_fn: Callable[[str, int, dict[str, Any]], ProviderResult] | None = None, started_at: str | None = None, run_id: str | None = None) -> dict[str, Any]:
    ...
    for line_id in config["required_count_lines"]:
        ...
        audit, rows = _run(expressions[line_id], fetch_limit, f"gf02_{version_slug}_{line_id.replace('#', 'line_')}", checkpoint_dir, search_fn, mode)
        ...
        if line_id == "#7":
            snap = _write_jsonl(run_dir / f"{version_slug}_final.rows.jsonl", rows)
    ...
    rescue, rescue_rows = _run(rescue_query, int(limit), f"gf02_{version_slug}_rescue_only", checkpoint_dir, search_fn, "RESCUE_ONLY_FULL_CAPPED")
    rescue_snap = _write_jsonl(run_dir / f"{version_slug}_rescue_only.rows.jsonl", rescue_rows)
    ...
    sample_path, actual_sample = _write_sample(run_dir / f"rescue_only_sample_{version_slug}.csv", rescue_rows, sample_size, noise_seed, rescue["total_found"], rescue["records_returned"], rescue["rows_capped"], candidate_version=candidate_version)
    ...
    final = audits["#7"]
    manifest = {
        "schema_version": 3, "run_id": resolved_run_id, "route_id": config["route_id"], "candidate_version": candidate_version,
        ...
        "final_pubmed_provider_meta": final["provider_meta"], "pubmed_advanced_search_details_required": True, "rescue_only": rescue,
        "rescue_only_sample": str(sample_path), "rescue_only_sample_sha256": _hash_file(sample_path),
        "rescue_only_sampling_rule": {"seed": noise_seed, "requested_size": sample_size, "actual_size": actual_sample, "minimum_required": int(sample_cfg["minimum"]), "maximum_allowed": int(sample_cfg["maximum"]), "human_classification_required": True, "representative_of_full_rescue_only": not bool(rescue["rows_capped"])},
        ...
        "scientific_interpretation_allowed": False, "ready_for_press_inferred": False, "press_approved": False, "freeze_authorized": False,
    }
