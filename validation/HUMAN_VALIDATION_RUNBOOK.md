...
The canonical production path generates opaque assessor slots at runtime. It does not require names, e-mails, GitHub accounts or fixed `assessor_A` / `assessor_B` identities in source control.
...
A compatibility path may supply repeated `--assessor-id` values only when they are already opaque, non-identifying runtime IDs. Synthetic labels such as `assessor_A` may remain in unit tests/examples but are not the canonical production identity model. Do not pass person names, e-mail addresses, usernames or account identifiers.
