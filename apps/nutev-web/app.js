const state={providers:[],last:null,currentJob:null,currentMode:null};
const $=s=>document.querySelector(s);
const $$=s=>[...document.querySelectorAll(s)];
const GLOBAL_PER_PROVIDER=100;
const GLOBAL_MAX_RESULTS=300;
...
function activateGlobalSearchControls(){
  $$('#providerGrid input').forEach(input=>{input.checked=true});
  $('#perProvider').value=String(GLOBAL_PER_PROVIDER);
  $('#maxResults').value=String(GLOBAL_MAX_RESULTS);
}
...
async function runSearch(options={}){
  ...
  const searchBtn=$('#searchBtn'),globalBtn=$('#globalSearchBtn');searchBtn.disabled=true;globalBtn.disabled=true;state.currentJob=null;state.currentMode=global?'global':'custom';$('#summary').classList.add('hidden');$('#results').innerHTML='';
  $('#searchState').className='loading';$('#searchState').innerHTML=global?'<strong>Iniciando Busca global…</strong><div class="small-state">Todas as fontes conectadas · profundidade ampliada</div>':'Iniciando busca…';
  try{
    const res=await fetch('/api/search/jobs',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({query,providers,per_provider:global?GLOBAL_PER_PROVIDER:Number($('#perProvider').value||25),max_results:global?GLOBAL_MAX_RESULTS:Number($('#maxResults').value||100)})});
...
function renderJob(job){
  ...
  const prefix=state.currentMode==='global'?'Busca global · ':'';
  ...
  $('#searchState').innerHTML=`<div><strong>${esc(prefix+stage)}</strong></div><div class="provider-status">${(job.providers||[]).map(p=>`<span class="provider-badge ${badgeClass(p.status)}" title="${esc(p.error||'')}">${esc(p.label)} · ${esc(statusLabel(p.status))}${['queued','running'].includes(p.status)?'':` · ${Number(p.returned||0)}`}</span>`).join('')}</div>`;
}
