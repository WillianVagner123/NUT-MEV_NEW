...
function saveComparison(card){
  ...
  const relation=card.querySelector('[data-relation]').value;const rationale=card.querySelector('[data-rationale]').value.trim();const statusNode=card.querySelector('[data-save-state]');
  if(!relation){statusNode.textContent='Selecione a relação antes de salvar.';return}if(rationale.length<20){statusNode.textContent='Registre uma justificativa com pelo menos 20 caracteres.';return}
  ...
  state.draft.decisions[id]={decision_id:id,domain:state.domain,domain_label:DOMAIN_LABELS[state.domain]||state.domain,anchor:sourceSnapshot(anchor),candidate:sourceSnapshot(candidate),comparability,relation,rationale,reviewer:state.draft.reviewer||'',reviewed_at:reviewedAt,human_entered:true,canonical:false};saveDraft();renderQueue();renderLedger()
}
